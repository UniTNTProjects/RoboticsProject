...
from components.controller_manager import ControllerManager
import time
...
from utils.common_functions import plotJoint, plotEndeff
...
class Ur5Generic(BaseControllerFixed):
    ...
    def loadModelAndPublishers(self, xacro_path):
        ...
        # subscriber for gripper CPP
        self.pub_gripper = ros.Subscriber(
            "/" + self.robot_name + "/gripper_controller/command", Int32, callback=self._receive_gripper, queue_size=1)
        ...
        self.sub_pointcloud = ros.Subscriber(
            "/ur5/zed_node/point_cloud/cloud_registered", PointCloud2,   callback=self.receive_pointcloud, queue_size=1)

    def _receive_gripper(self, msg):
        gripper_diameter = int(msg.data)
        print("----------------------------------------------\nreceived gripper diameter: ", gripper_diameter)

        if self.real_robot:
            # directly send the command to the gripper
            self.controller_manager.gm.move_gripper(gripper_diameter)
            return

        gripper_dest = self.controller_manager.gm.mapToGripperJoints(
            gripper_diameter)
        ...
        while time < max_time and abs(gripper_dest-self.controller_manager.gm.getDesGripperJoints()[0]) > 0.01:
            self.controller_manager.gm.move_gripper(gripper_diameter)
            # send updated gripper position to publisher by sending same position again
            self.controller_manager.send_reduced_des_jstate(self.q)

            rate.sleep()
            time += conf.robot_params[p.robot_name]['dt']
    ...
    def updateKinematicsDynamics(self):
        ...
        self.x_c = self.robot.framePlacement(
            self.q, self.robot.model.getFrameId(pointcloud_frame)).translation
        self.w_R_c = self.robot.framePlacement(
            self.q, self.robot.model.getFrameId(pointcloud_frame)).rotation
...
def talker(p):
    ...
    if p.real_robot:
        ...
        additional_args = ['gripper:=' + str(p.gripper), 'soft_gripper:=' + str(
            conf.robot_params[p.robot_name]['soft_gripper'])]  # , 'gui:=false']
    ...
    if p.homing_flag:
        if p.real_robot:
            ...
            v_des = 0.6
        p.homing_procedure(
            conf.robot_params[p.robot_name]['dt'], v_des, conf.robot_params[p.robot_name]['q_0'], rate)
    ...
    while not ros.is_shutdown():
        ...
        if p.real_robot:
            p.ros_pub.add_arrow(
                p.x_ee + p.base_offset, p.contactForceW / (6 * p.robot.robot_mass), "green")
