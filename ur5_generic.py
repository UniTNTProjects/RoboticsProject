...
class Ur5Generic(BaseControllerFixed):
    def __init__(self, robot_name="ur5"):
        ...
        print(
            "Initialized ur5 generic  controller---------------------------------------------------------------"
        )
    ...
    def loadModelAndPublishers(self, xacro_path):
        ...
        self.pub_gripper = ros.Subscriber(
            "/" + self.robot_name + "/gripper_controller/command",
            Int32,
            callback=self._receive_gripper,
            queue_size=1,
        )
    ...
    def _receive_gripper(self, msg):
        ...
        gripper_dest = self.controller_manager.gm.mapToGripperJoints(gripper_diameter)
        ...
        while (
            time < max_time
            and abs(gripper_dest - self.controller_manager.gm.getDesGripperJoints()[0])
            > 0.01
        ):
            self.controller_manager.gm.move_gripper(gripper_diameter)
    ...
    def updateKinematicsDynamics(self):
        ...
        self.x_c = self.robot.framePlacement(
            self.q, self.robot.model.getFrameId(pointcloud_frame)
        ).translation
...
def talker(p):
    ...
    xacro_path = (
        rospkg.RosPack().get_path("ur_description")
        + "/urdf/"
        + p.robot_name
        + ".urdf.xacro"
    )
