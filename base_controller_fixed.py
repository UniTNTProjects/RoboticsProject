...
class BaseControllerFixed(threading.Thread):
    ...
    def startSimulator(self, world_name=None, use_torque_control=True,  additional_args=None, launch_file=None):
        ...
        custom_models_path = rospkg.RosPack().get_path(
            'ros_impedance_controller')+"/worlds/models/"
        ...
        if launch_file is None:
            launch_file = rospkg.RosPack().get_path('ros_impedance_controller') + \
                '/launch/ros_impedance_controller_' + self.robot_name + '.launch'

        # clean up previous process
        os.system("killall rosmaster rviz gzserver gzclient")
        if (distro.linux_distribution()[1] == "16.04"):
            print(colored(
                "This file only works with distribution from ROS lunar (I.e. ubuntu 17.04 or later) ", "red"))
        uuid = roslaunch.rlutil.get_or_generate_uuid(None, False)
        roslaunch.configure_logging(uuid)
        cli_args = [launch_file,
                    'spawn_x:=' +
                    str(conf.robot_params[self.robot_name]['spawn_x']),
                    'spawn_y:=' +
                    str(conf.robot_params[self.robot_name]['spawn_y']),
    ...
    def loadModelAndPublishers(self,  xacro_path=None, additional_urdf_args=None):
        ...
        if xacro_path is None:
            ...
            print(
                colored(f"loading custom xacro path:  : {xacro_path}", "blue"))
    ...
    def startupProcedure(self):
        if (self.use_torque_control):
            if ("/" + self.robot_name + "/ros_impedance_controller" not in rosnode.get_node_names()):
                print(colored(
                    'Error: you need to launch the ros impedance controller in torque mode!', 'red'))
                sys.exit()
    ...
    def initVars(self):
        ...
        # log vars
        self.q_des_log = np.empty(
            (self.robot.na, conf.robot_params[self.robot_name]['buffer_size']))*nan
        self.q_log = np.empty(
            (self.robot.na, conf.robot_params[self.robot_name]['buffer_size'])) * nan
        self.qd_des_log = np.empty(
            (self.robot.na, conf.robot_params[self.robot_name]['buffer_size']))*nan
        self.qd_log = np.empty(
            (self.robot.na, conf.robot_params[self.robot_name]['buffer_size'])) * nan
        self.tau_ffwd_log = np.empty(
            (self.robot.na, conf.robot_params[self.robot_name]['buffer_size']))*nan
        self.tau_log = np.empty(
            (self.robot.na, conf.robot_params[self.robot_name]['buffer_size']))*nan

        self.x_ee_log = np.empty(
            (3, conf.robot_params[self.robot_name]['buffer_size'])) * nan
        self.x_ee_des_log = np.empty(
            (3, conf.robot_params[self.robot_name]['buffer_size'])) * nan

        self.contactForceW_log = np.empty(
            (3, conf.robot_params[self.robot_name]['buffer_size'])) * nan
        self.time_log = np.empty(
            (conf.robot_params[self.robot_name]['buffer_size']))*nan

        self.log_counter = 0

        self.ikin = robotKinematics(
            self.robot, conf.robot_params[self.robot_name]['ee_frame'])
...
def talker(p):
    ...
    p.startSimulator()
    if (robotName == 'ur5'):
        p.loadModelAndPublishers(rospkg.RosPack().get_path(
            'ur_description') + '/urdf/' + p.robot_name + '.urdf.xacro')
    else:
