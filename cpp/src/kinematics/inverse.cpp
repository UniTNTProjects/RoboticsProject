...
auto T65f =  [&](double th6) ->homoMatrix{
 ...
 mat << 0.0, 0.0, -D[5], 1.0;
 p50 << T60 * mat;
 th1_1 = real(atan2(p50(2), p50(1)) + acos(D[3]/hypot(p50(2), p50(1))))+pi/2;
 th1_2 = real(atan2(p50(2), p50(1)) - acos(D[3]/hypot(p50(2), p50(1))))+pi/2;
 
 //finding th5
 double th5_1 = real(acos((p60(1)*sin(th1_1) - p60(2)*cos(th1_1)-D[3]) / D[5]));
 double th5_2 = -real(acos((p60(1)*sin(th1_1) - p60(2)*cos(th1_1)-D[3]) / D[5]));
 double th5_3 = real(acos((p60(1)*sin(th1_2) - p60(2)*cos(th1_2)-D[3]) / D[5]));
 double th5_4 = -real(acos((p60(1)*sin(th1_2) - p60(2)*cos(th1_2)-D[3]) / D[5]));
