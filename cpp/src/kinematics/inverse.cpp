...
auto T65f =  [&](double th6) ->homoMatrix{
 ...
 mat << 0.0, 0.0, -D[5], 1.0;
 p50 << T60 * mat;
 th(0,0) = real(atan2(p50[1], p50[0])) * complex_converter + acos(D[3] / (sqrt(pow(p50[1], 2) + pow(p50[0], 2)))) + M_PI/2;
 th(0,1) = real(atan2(p50[1], p50[0])) * complex_converter + acos(D[3] / (sqrt(pow(p50[1], 2) + pow(p50[0], 2)))) + M_PI/2;
...
int main(){
    ...
    jointValues inv = ur5Inverse(pe, re);
    cout << "Joint angles: \n" << inv << endl;
