...
# globals
labels_ur = ["1 - Shoulder Pan", "2 - Shoulder Lift",
             "3 - Elbow", "4 - Wrist 1", "5 - Wrist 2", "6 - Wrist 3"]
labels_quadruped = ["LF_HAA", "LF_HFE", "LF_KFE", "LH_HAA", "LH_HFE",
                    "LH_KFE", "RF_HAA", "RF_HFE", "RF_KFE", "RH_HAA", "RH_HFE", "RH_KFE"]
labels_flywheel2 = labels_quadruped.append(["left_wheel", "right_wheel"])
labels_flywheel4 = labels_quadruped.append(
...
def startNode(node_name):
    ...
    node = roslaunch.core.Node(
        package, executable, name, namespace, output="screen")
...
def getRobotModel(robot_name="hyq", generate_urdf=False, xacro_path=None, additional_urdf_args=None):
    ...
    if (generate_urdf):
        try:
            ...
            args = xacro_path + ' --inorder -o ' + \
                os.environ['LOCOSIM_DIR'] + \
                '/robot_urdf/generated_urdf/'+robot_name+'.urdf'
...
def subplot(n_rows, n_cols, n_subplot, sharex=False, sharey=False, ax_to_share=None):
    if sharex and sharey:
        ax = plt.subplot(n_rows, n_cols, n_subplot,
                         sharex=ax_to_share, sharey=ax_to_share)
    ...
    for jidx in range(njoints):
        if (njoints % 3 == 0):  # divisible by 3
            if jidx == 0:
                ...
                subplot(n_rows, n_cols, jidx + 1, sharex=sharex,
                        sharey=sharey, ax_to_share=ax)
        ...
        if (plot_var_des_log is not None):
            plt.plot(time_log[start:end], plot_var_des_log[jidx, start:end],
                     linestyle='-', marker="o", markersize=marker_size, lw=lw_des, color='red')
        if (plot_var_log is not None):
            plt.plot(time_log, plot_var_log[jidx, :], linestyle='-',
                     marker="o", markersize=marker_size, lw=lw_act, color='blue')
        ...
        if (q_adm is not None):
            plt.plot(time_log[start:end], q_adm[jidx, start:end], linestyle='-',
                     marker="o", markersize=marker_size, lw=lw_act, color='black')
        plt.grid()
...
def plotFrame(name, time_log, des_Pose_log=None, Pose_log=None, des_Twist_log=None, Twist_log=None, des_Acc_log=None, Acc_log=None,
    ...
    if (plot_var_des_log is not None):
        plt.plot(time_log[start:end], plot_var_des_log[0, start:end], linestyle='-',
                 marker="o", markersize=marker_size, lw=lw_des, color='red')
    if plot_var_log is not None:
        plt.plot(time_log[start:end], plot_var_log[0, start:end], linestyle='-',
                 marker="o", markersize=marker_size, lw=lw_act, color='blue')
    plt.grid()
    ...
    if (plot_var_des_log is not None):
        plt.plot(time_log[start:end], plot_var_des_log[1,
                 start:end], linestyle='-', lw=lw_des, color='red')
    if plot_var_log is not None:
    ...
    plt.xlabel("Time [s]")
    if (plot_var_des_log is not None):
        plt.plot(time_log[start:end], plot_var_des_log[2,
                 start:end], linestyle='-', lw=lw_des, color='red')
    if plot_var_log is not None:
    ...
    if (plot_var_des_log is not None):
        plt.plot(time_log[start:end], plot_var_des_log[3,
                 start:end], linestyle='-', lw=lw_des, color='red')
    if plot_var_log is not None:
    ...
    if (plot_var_des_log is not None):
        plt.plot(time_log[start:end], plot_var_des_log[4,
                 start:end], linestyle='-', lw=lw_des, color='red')
    if plot_var_log is not None:
    ...
    plt.xlabel("Time [s]")
    if (plot_var_des_log is not None):
        plt.plot(time_log[start:end], plot_var_des_log[5,
                 start:end], linestyle='-', lw=lw_des, color='red')
    if plot_var_log is not None:
...
def plotContacts(name, time_log, des_LinPose_log=None, LinPose_log=None, des_LinTwist_log=None, LinTwist_log=None, des_Forces_log=None,
    ...
    if plot_var_log is not None:
        plt.plot(time_log[start:end], plot_var_log[idx,
                 start:end], linestyle='-', lw=lw_act, color='blue')
    if des_LinPose_log is not None:
        plt.plot(time_log[start:end], des_LinPose_log[idx,
                 start:end], linestyle='-', lw=lw_des, color='red')
    if name == 'GRFs' and gt_Forces_log is not None:
        plt.plot(time_log[start:end], gt_Forces_log[idx,
                 start:end], linestyle='-', lw=lw_act, color='green')
    if contact_states is not None:
        ax2 = ax.twinx()
        plt.plot(time_log[start:end], contact_states[idx,
                 start:end], linestyle='-', lw=2, color='black')
        ax2.grid()

    ax1 = subplot(6, 2, 3, sharex=sharex, sharey=sharey, ax_to_share=ax)
    plt.ylabel("$LF_y " + unit + "$", fontsize=10)
    if plot_var_log is not None:
        plt.plot(time_log[start:end], plot_var_log[idx+1,
                 start:end], linestyle='-', lw=lw_act, color='blue')
    if des_LinPose_log is not None:
        plt.plot(time_log[start:end], des_LinPose_log[idx+1,
                 start:end], linestyle='-', lw=lw_des, color='red')
    if name == 'GRFs' and gt_Forces_log is not None:
        plt.plot(time_log[start:end], gt_Forces_log[idx+1,
                 start:end], linestyle='-', lw=lw_act, color='green')
    if contact_states is not None:
        ax2 = ax.twinx()
        plt.plot(time_log[start:end], contact_states[idx,
    ...
    if plot_var_log is not None:
        plt.plot(time_log[start:end], plot_var_log[idx+2,
                 start:end], linestyle='-', lw=lw_act, color='blue')
    if des_LinPose_log is not None:
        plt.plot(time_log[start:end], des_LinPose_log[idx+2,
                 start:end], linestyle='-', lw=lw_des, color='red')
    if name == 'GRFs' and gt_Forces_log is not None:
        plt.plot(time_log[start:end], gt_Forces_log[idx+2,
                 start:end], linestyle='-', lw=lw_act, color='green')
    if contact_states is not None:
        ax2 = ax.twinx()
        plt.plot(time_log[start:end], contact_states[idx,
...
#    fig = plt.figure(figure_id)
    ...
    axs[0, 0].plot((x_log[0, :].T-x_des_log[0, :].T),
                   f_log[0, :].T, lw=lw_act, color='blue')
    axs[0, 0].set_title('Fx vs X')
    axs[0, 0].grid()

    axs[0, 1].plot((x_log[1, :].T-x_des_log[1, :].T),
                   f_log[0, :].T, lw=lw_act, color='blue')
    axs[0, 1].set_title('Fx vs Y')
    axs[0, 1].grid()

    axs[0, 2].plot((x_log[2, :].T-x_des_log[2, :].T),
                   f_log[0, :].T, lw=lw_act, color='blue')
    axs[0, 2].set_title('Fx vs Z')
    axs[0, 2].grid()

    axs[1, 0].plot((x_log[0, :].T-x_des_log[0, :].T),
                   f_log[1, :].T, lw=lw_act, color='blue')
    axs[1, 0].set_title('Fy vs X')
    axs[1, 0].grid()

    axs[1, 1].plot((x_log[1, :].T-x_des_log[1, :].T),
                   f_log[1, :].T, lw=lw_act, color='blue')
    axs[1, 1].set_title('Fy vs Y')
    axs[1, 1].grid()

    axs[1, 2].plot((x_log[2, :].T-x_des_log[2, :].T),
                   f_log[1, :].T, lw=lw_act, color='blue')
    axs[1, 2].set_title('Fy vs Z')
    axs[1, 2].grid()

    axs[2, 0].plot((x_log[0, :].T-x_des_log[0, :].T),
                   f_log[2, :].T, lw=lw_act, color='blue')
    axs[2, 0].set_title('Fz vs X')
    axs[2, 0].grid()

    axs[2, 1].plot((x_log[1, :].T-x_des_log[1, :].T),
                   f_log[2, :].T, lw=lw_act, color='blue')
    axs[2, 1].set_title('Fz vs Y')
    axs[2, 1].grid()

    axs[2, 2].plot((x_log[2, :].T-x_des_log[2, :].T),
                   f_log[2, :].T, lw=lw_act, color='blue')
...
def plotJointImpedance(name, q_log, q_des_log, tau_log):
    ...
    labels_ur = ["1 - Shoulder Pan", "2 - Shoulder Lift",
                 "3 - Elbow", "4 - Wrist 1", "5 - Wrist 2", "6 - Wrist 3"]
    labels_hyq = ["LF_HAA", "LF_HFE", "LF_KFE", "RF_HAA", "RF_HFE",
                  "RF_KFE", "LH_HAA", "LH_HFE", "LH_KFE", "RH_HAA", "RH_HFE", "RH_KFE"]
    ...
    for jidx in range(njoints):
        ...
        plt.plot(q_log[jidx, :].T-q_des_log[jidx, :].T,
                 tau_log[jidx, :].T, linestyle='-', lw=lw_des, color='blue')
        plt.grid()
...
def plotWrenches(name, figure_id, time_log, wrench_fb_log=None, wrench_ffwd_log=None, wrench_g_log=None):
    ...
    plt.subplot(3, 2, 1)
    plt.ylabel(labels[0])
    plt.plot(time_log, plot_var_des_log[0, :], linestyle='-',
             marker="o", markersize=marker_size, lw=lw_des, color='red')
    plt.grid()

    plt.subplot(3, 2, 3)
    plt.ylabel(labels[1])
    plt.plot(time_log, plot_var_des_log[1, :], linestyle='-',
             marker="o", markersize=marker_size, lw=lw_des, color='red')
    plt.grid()
    ...
    plt.ylabel(labels[2])
    plt.xlabel("Time [s]")
    plt.plot(time_log, plot_var_des_log[2, :], linestyle='-',
             marker="o", markersize=marker_size, lw=lw_des, color='red')
    plt.grid()

    plt.subplot(3, 2, 2)
    plt.ylabel(labels[3])
    plt.plot(time_log, plot_var_des_log[3, :], linestyle='-',
             marker="o", markersize=marker_size, lw=lw_des, color='red')
    plt.grid()

    plt.subplot(3, 2, 4)
    plt.ylabel(labels[4])
    plt.plot(time_log, plot_var_des_log[4, :], linestyle='-',
             marker="o", markersize=marker_size, lw=lw_des, color='red')
    plt.grid()
    ...
    plt.ylabel(labels[5])
    plt.xlabel("Time [s]")
    plt.plot(time_log, plot_var_des_log[5, :], linestyle='-',
             marker="o", markersize=marker_size, lw=lw_des, color='red')
    plt.grid()
