...
class Math:
    ...
    def rot2eul(self, R):
        ...
        theta = np.arctan2(-R[2, 0],
                           np.sqrt(pow(R[2, 1], 2) + pow(R[2, 2], 2)))
    ...
    def point_is_right_of_line(self, p0, p1, p2):
        ...
        return (p2[utils.crd["X"]] - p0[utils.crd["X"]]) * (p1[utils.crd["Y"]] - p0[utils.crd["Y"]]) \
            - (p1[utils.crd["X"]] - p0[utils.crd["X"]]) * \
            (p2[utils.crd["Y"]] - p0[utils.crd["Y"]])
    ...
    def margin_from_poly(self, point_to_test, stance_legs, actual_feetW):
        ...
        for leg in range(4):
            if (stance_legs[leg]):
                ...
                stance_feetW.append(
                    np.array([actual_feetW[leg][0], actual_feetW[leg][1], 0.0]))
    ...
    def compute_half_plane_description(self, vertices):
        ...
        for vertix in range(0, number_of_constraints):
            ...
            print(vertices_ccwise_sorted[vertix], vertices_ccwise_sorted[(
                vertix + 1) % number_of_constraints])
    ...
    def is_point_inside_segment(self, first_input_point, second_input_point, point_to_check):
        ...
        if (np.abs(first_input_point[0] - second_input_point[0]) < 1e-02):
            alpha = (point_to_check[1] - second_input_point[1]) / \
                (first_input_point[1] - second_input_point[1])
        else:
            alpha = (point_to_check[0] - second_input_point[0]) / \
                (first_input_point[0] - second_input_point[0])
    ...
    def find_point_to_line_signed_distance(self, segment_point1, segment_point2, point_to_check):
        ...
        num = (segment_point2[0] - segment_point1[0])*(segment_point1[1] - point_to_check[1]) - (
            segment_point1[0] - point_to_check[0])*(segment_point2[1] - segment_point1[1])
        denum_sq = (segment_point2[0] - segment_point1[0])*(segment_point2[0] - segment_point1[0]) + (
            segment_point2[1] - segment_point1[1])*(segment_point2[1] - segment_point1[1])
...
#        print segment_point1, segment_point2, point_to_check, dist
    ...
    def find_residual_radius(self, polygon, point_to_check):
        ...
        for i in range(0, numberOfVertices-1):
            ...
            d_temp = self.find_point_to_line_signed_distance(
                s1, s2, point_to_check)
#            print i, s1, s2, d_temp
            if d_temp < 0.0:
                print(
                    'Warning! found negative distance. Polygon might not be in clockwise order...')
...
#        print polygon[numberOfVertices-1,:], polygon[0,:], d_temp
    ...
    def find_polygon_segment_intersection(self, vertices_input, desired_direction, starting_point):
        desired_direction = desired_direction / \
            np.linalg.norm(desired_direction)*10.0
        # print "desired dir: ", desired_direction

        desired_com_line = self.line(
            starting_point, starting_point+desired_direction)
        ...
        for i in range(0, len(vertices_input)):
            ...
            new_point = self.two_lines_intersection(
                desired_com_line, actuation_region_edge)

            if new_point:
                intersection_points = np.vstack(
                    [intersection_points, new_point])
                new_point, alpha = self.is_point_inside_segment(
                    starting_point, starting_point+desired_direction, new_point)
                if new_point:
                    points_along_direction = np.vstack(
                        [points_along_direction, new_point])
                    d = np.sqrt((new_point[0] - starting_point[0])*(new_point[0] - starting_point[0]) + (
                        new_point[1] - starting_point[1])*(new_point[1] - starting_point[1]))
                    point_to_com_distance = np.vstack(
                        [point_to_com_distance, d])
...
def motionVectorTransform(position, rotationMx):
    utils = Utils()
    b_X_a = np.zeros((6, 6))
    b_X_a[utils.sp_crd["AX"]:utils.sp_crd["AX"] + 3,
          utils.sp_crd["AX"]: utils.sp_crd["AX"] + 3] = rotationMx
    b_X_a[utils.sp_crd["LX"]:utils.sp_crd["LX"] + 3,   utils.sp_crd["AX"]: utils.sp_crd["AX"] + 3] = -rotationMx*cross_mx(position)
    b_X_a[utils.sp_crd["LX"]:utils.sp_crd["LX"] + 3,
          utils.sp_crd["LX"]: utils.sp_crd["LX"] + 3] = rotationMx
    return b_X_a
...
def polynomialRef(x0, xf, v0, vf, a0, af, T):
    ...
    v_coeffs = np.array([p_coeffs[1], 2 * p_coeffs[2], 3 * p_coeffs[3],
                        4 * p_coeffs[4], 5 * p_coeffs[5], 6 * p_coeffs[6], 7 * p_coeffs[7]])
    a_coeffs = np.array([v_coeffs[1], 2 * v_coeffs[2], 3 * v_coeffs[3],
                        4 * v_coeffs[4], 5 * v_coeffs[5], 6 * v_coeffs[6]])

    def pos(t): return (p_coeffs[0] +
                        p_coeffs[1] * t +
                        p_coeffs[2] * t ** 2 +
                        p_coeffs[3] * t ** 3 +
                        p_coeffs[4] * t ** 4 +
